...
def seed_if_empty():
    ...
    # 각 건마다 발송 상태를 다르게 줘서, 화면을 처음 열었을 때부터 발송실패→재발송
    # 같은 기능을 바로 만져볼 수 있게 한다.
    past_statuses = [
        Notification.STATUS_SENT,
        Notification.STATUS_SENT,
        Notification.STATUS_FAILED,  # 재발송 버튼 데모용
        Notification.STATUS_SENT,
    ]
    for (u, dest, _, _), status in zip(employee_users[:4], past_statuses):
